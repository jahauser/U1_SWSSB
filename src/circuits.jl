...
function update_data(state::DiagonalState, observables::Vector{Symbol}, data::Dict{Symbol,Vector}, t::Int)
    ψ = state.mps
    L = length(ψ)
    if :κ2_ratio in observables
        corrs = χ2_corr(ψ)
...
function circuit(state::DiagonalState, L::Int, T::Int, γ::Float64, λ::Float64; observables=Symbol[], terminal_observables=Symbol[],
    ...
    for t in 1:T
        ...
        data = update_data(state, observables, data, 2t+1)

        if t == T
            terminal_data = update_data(state, terminal_observables, terminal_data, 1)
        end
        ...
        if λ > 0.0
            ...
            state /= norm(state)
            truncate!(state; cutoff=cutoff, maxdim=maxdim)
            data = update_data(state, observables, data, 2t+2)
        end

        if t == T
            terminal_data = update_data(state, terminal_observables, terminal_data, 2)
        end
    end
...
function circuit(state::DiagonalState, L::Int, T::Int, γ::Float64, λ::Float64, samples::Int; observables=Symbol[],
    terminal_observables=Symbol[], PBC=false, cutoff=1E-8, maxdim=200)

    full_data = initiate_data(state, observables, L, T)
    full_terminal_data = initiate_data(state, terminal_observables, L, 2)

    for _ in 1:samples
        _, data, terminal_data = circuit(state, L, T, γ, λ; observables=observables, PBC=PBC, cutoff=cutoff, maxdim=maxdim)
